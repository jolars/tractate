...
use super::lowering::{preserve, range};
use crate::document::*;

pub(super) fn lower(node: &cst::SyntaxNode) -> YamlValue {
    ...
    let kind = match node.kind() {
        ...
        SyntaxKind::YAML_BLOCK_MAP | SyntaxKind::YAML_FLOW_MAP => {
            let entries = node
                ...
                .map(|entry| {
                    ...
                    YamlEntry {
                        range: range(entry.text_range()),
                        key: key
                            .as_ref()
                            .map(lower)
                            .unwrap_or_else(|| empty(range(entry.text_range()))),
                        value: value.as_ref().map(lower).unwrap_or_else(|| {
                            empty(SourceRange {
        ...
        SyntaxKind::YAML_BLOCK_SEQUENCE | SyntaxKind::YAML_FLOW_SEQUENCE => {
            let items = node
                ...
                .filter(|child| {
                    matches!(
                        child.kind(),
                        SyntaxKind::YAML_BLOCK_SEQUENCE_ITEM | SyntaxKind::YAML_FLOW_SEQUENCE_ITEM
                    )
                })
                .map(|item| lower(&item))
                .collect();
        ...
        _ => {
            ...
            if let Some(content) = content {
                ...
                YamlKind::Unsupported(preserve(node.clone().into()))
            }
        }
    };
    YamlValue {
...
fn empty(range: SourceRange) -> YamlValue {
    YamlValue {
        range,
        properties: Vec::new(),
        kind: YamlKind::Empty,
